...
class NicknameAPITestCase(TestCase):
    ...
    def test_nickname_change_with_valid_request(self):
        ...
        response = self.client.post(
            self.nickname_url, valid_data, format='json')
        response_data = response.json()
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response_data.get('code'), 0)
        self.assertEqual(
...
class TimelineAPITestCase(TestCase):
    ...
    def test_timeline_change_with_valid_request(self):
        ...
        response = self.client.post(
            self.timeline_url, valid_data, format='json')
        response_data = response.json()
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response_data.get('code'), 0)
        self.assertEqual(
            response_data.get('message'),
            "Timeline successfully changed!")
...
class RecordAPITestCase(TestCase):
    ...
    def test_record_change_with_valid_request(self):
        ...
        self.assertEqual(
            response_data.get('message'),
            "Record button successfully changed!")
...
class WordbookCreateAPITestCase(TestCase):
    ...
    def test_create_wordbook_with_valid_request(self):
        ...
        self.assertEqual(
            response_data.get('message'),
            "New wordbook successfully created!")
...
class WordbookTypeChangeAPITestCase(TestCase):
    ...
    def test_change_wordbook_type_with_valid_request(self):
        ...
        response = self.client.post(
            self.change_type_url, valid_data, format='json')
        response_data = response.json()
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response_data.get('code'), 0)
        self.assertEqual(
            response_data.get('message'),
            "Wordbook information successfully changed!")
    ...
    def test_change_wordbook_type_with_nonexistent_wordbook(self):
        ...
        测试更改不存在的单词本公开属性。
        """
        nonexistent_wordbook_data = {
            "openid": "3e5428-ff58yj5", "index": 999, "type": 0}
        response = self.client.post(
            self.change_type_url,
            nonexistent_wordbook_data,
            format='json')
...
class WordAddAPITestCase(TestCase):
    ...
    def test_add_word_with_valid_request(self):
        ...
        response = self.client.post(
            self.add_word_url, valid_data, format='json')
        response_data = response.json()
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertIn(response_data.get('code'), [0])
        self.assertIn(
            response_data.get('message'), [
                "Word added successfully!", "Word already exists"])
...
class WordRemoveAPITestCase(TestCase):
    ...
    def test_remove_word_with_valid_request(self):
        ...
        valid_data = {"openid": "xxxx", "index": 2, "word": "あかい…"}
        response = self.client.post(
            self.remove_word_url, valid_data, format='json')
        response_data = response.json()
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response_data.get('code'), 0)
        self.assertEqual(
...
class FriendsListAPITestCase(TestCase):
    ...
    def test_get_friends_list_with_valid_request(self):
        ...
        valid_data = {"openid": "3e5428-ff58yj5"}
        response = self.client.post(
            self.friends_list_url, valid_data, format='json')
        response_data = response.json()
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response_data.get('code'), 0)
...
class FriendsHeadIconAPITestCase(TestCase):
    ...
    def test_get_headicon_with_valid_request(self):
        ...
        response = self.client.post(
            self.headicon_url, valid_data, format='json')
        response_data = response.json()
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response_data.get('code'), 0)
...
class FriendsUnfollowAPITestCase(TestCase):
    ...
    def test_unfollow_with_valid_request(self):
        ...
        valid_data = {"openid": "3e5428-ff58yj5", "uid": "10000001"}
        response = self.client.post(
            self.unfollow_url, valid_data, format='json')
        response_data = response.json()
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response_data.get('code'), 0)
...
class FriendsUidSearchAPITestCase(TestCase):
    ...
    def test_uidsearch_with_valid_request(self):
        ...
        valid_data = {"openid": "3e5428-ff58yj5 ", "uid": "10000001"}
        response = self.client.post(
            self.uidsearch_url, valid_data, format='json')
        response_data = response.json()
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response_data.get('code'), 0)
        self.assertIsInstance(response_data.get('data').get('result'), list)
...
class FriendsNameSearchAPITestCase(TestCase):
    ...
    def test_namesearch_with_valid_request(self):
        ...
        response = self.client.post(
            self.namesearch_url, valid_data, format='json')
        response_data = response.json()
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response_data.get('code'), 0)
        self.assertIsInstance(response_data.get('data').get('result'), list)
...
class WordbookInfoChangeAPITestCase(TestCase):
    def setUp(self):
        self.client = APIClient()
        self.info_url = reverse('wb_info_change')

    def test_change_wordbook_info_with_valid_request(self):
        """
        测试使用有效请求更改单词本信息。
        """
        valid_data = {
        ...
        response = self.client.post(self.info_url, valid_data, format='json')
        response_data = response.json()
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response_data.get('code'), 0)
        self.assertEqual(
    ...
    def test_change_wordbook_info_with_wrong_method(self):
        """
        测试使用错误的请求方式更改单词本信息。
        """
        wrong_method_data = {
    ...
    def test_change_wordbook_info_with_invalid_user(self):
        """
        测试使用不存在的用户更改单词本信息。
        """
        invalid_user_data = {
        ...
        response = self.client.post(
            self.info_url, invalid_user_data, format='json')
        response_data = response.json()
        self.assertEqual(response.status_code, status.HTTP_401_UNAUTHORIZED)
        self.assertEqual(response_data.get('code'), "401")
        self.assertEqual(response_data.get('message'), "User Unauthorized")

    def test_change_wordbook_info_for_nonexistent_wordbook(self):
        """
        测试更改不存在单词本的信息。
        """
        nonexistent_wordbook_data = {
...
class FriendsSubscribeAPITestCase(TestCase):
    def setUp(self):
        self.client = APIClient()
        # 请确保这里的'friends_subscribe'与您urls.py中的名称一致
        self.subscribe_url = reverse('frineds_subscribe')

    def test_subscribe_with_valid_request(self):
        """
        测试使用有效请求通过uid和单词本id订阅。
        """
        valid_data = {"openid": "3e5428-ff58yj5", "uid": "10000002", "id": "1"}
        response = self.client.post(
            self.subscribe_url, valid_data, format='json')
        response_data = response.json()
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response_data.get('code'), 0)
        self.assertEqual(
    ...
    def test_subscribe_with_wrong_method(self):
        """
        测试使用错误的请求方式进行订阅。
        """
        wrong_method_data = {
    ...
    def test_subscribe_with_invalid_user(self):
        ...
        测试使用不存在的用户进行订阅。
        """
        invalid_user_data = {"openid": "xx", "uid": "invalid_user", "id": "1"}
        response = self.client.post(
            self.subscribe_url,
            invalid_user_data,
            format='json')
...
class FriendsUnsubscribeAPITestCase(TestCase):
    def setUp(self):
        self.client = APIClient()
        self.unsubscribe_url = reverse('friends_unsubscribe')

    def test_unsubscribe_with_valid_request(self):
        """
        测试使用有效请求通过uid和单词本id取消订阅。
        """
        valid_data = {"openid": "3e5428-ff58yj5", "uid": "10000002", "id": "1"}
        response = self.client.post(
            self.unsubscribe_url, valid_data, format='json')
        response_data = response.json()
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response_data.get('code'), 0)
        self.assertEqual(
    ...
    def test_unsubscribe_with_wrong_method(self):
        """
        测试使用错误的请求方式取消订阅。
        """
        wrong_method_data = {
    ...
    def test_unsubscribe_with_invalid_user(self):
        ...
        测试使用不存在的用户取消订阅。
        """
        invalid_user_data = {"openid": "22", "uid": "invalid_user", "id": "1"}
        response = self.client.post(
            self.unsubscribe_url,
            invalid_user_data,
            format='json')
...
class FriendsInfoAPITestCase(TestCase):
    def setUp(self):
        self.client = APIClient()
        self.info_url = reverse('friends_info')

    def test_get_info_with_valid_request(self):
        """
        测试使用有效请求通过uid搜索用户信息。
        """
        valid_data = {"openid": "3e5428-ff58yj5", "uid": "10000002"}
        response = self.client.post(self.info_url, valid_data, format='json')
        response_data = response.json()
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response_data.get('code'), 0)
        self.assertIn('info', response_data.get('data'))
        self.assertIn('wordbooks', response_data.get('data'))

    def test_get_info_with_wrong_method(self):
        """
        测试使用错误的请求方式搜索用户信息。
        """
        wrong_method_data = {"openid": "3e5428-ff58yj5", "uid": "10000002"}
        response = self.client.get(
            self.info_url,
            wrong_method_data,
            format='json')
    ...
    def test_get_info_with_invalid_user(self):
        """
        测试使用不存在的用户搜索信息。
        """
        invalid_user_data = {"openid": "ss", "uid": "invalid_user"}
        response = self.client.post(
            self.info_url, invalid_user_data, format='json')
        response_data = response.json()
        self.assertEqual(response.status_code, status.HTTP_401_UNAUTHORIZED)
