...
def word_add(request, json_param, user, user_wb):
    ...
    if len(exist_word) == 0:
        user_wb.wordlist.create(list_info=user_wb, content=new_word)     # 添加新单词
        return JsonResponse(
            {"message": "Word added successfully!"}, status=200)
    elif len(exist_word) == 1:
        return JsonResponse({"message": "Word already exists"}, status=403)
    else:
        return JsonResponse({'message': 'Duplicate word'}, status=403)
...
def word_remove(request, json_param, user, user_wb):
    ...
    if len(exist_word) == 0:
        ...
        exist_word[0].delete()     # 删除单词
        return JsonResponse(
            {"message": "Word removed successfully"}, status=200)
    else:
        for x in exist_word:
            x.delete()
        return JsonResponse({'message': 'Duplicate word'}, status=403)
